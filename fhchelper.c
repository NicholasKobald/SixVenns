...
/*
Fisher-yates shuffle. Places a uniformly
random permutation of [0, 1, 2... FMAX] in perm
*/
void generate_random_permutation(int perm[FMAX]){
	int i, j;
	int swap;
	for(i = FMAX; i>=0; i--){
		j=rand()%(i+1);
...
int load_line_from_file(FILE *fp, int G[NMAX][DEGREE]) {
	char line[1008];
	int i = 5;
	if(fgets(line, 1000, fp)==NULL) return 0;
	int temp;
	int vertice = 0;
	int j=0;
	while(vertice<NMAX) {
